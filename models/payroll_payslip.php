<?php
	...
	class payroll_payslip extends CI_Model {
	    ...
	    function fetchPayslipAddonDetails($empid,$stucId,$gen_date){
	    	...
	    	$sql="SELECT ID,PROFILE_ID,PAY_STRUCTURE_ID,(SELECT ID FROM employee_payslip WHERE EMP_PROFILE_ID=employee_profile.PROFILE_ID AND GENERATION_DATE='$gen_date') AS PAYSLIP_ID,(SELECT GENERATION_DATE FROM employee_payslip WHERE EMP_PROFILE_ID=employee_profile.PROFILE_ID AND GENERATION_DATE='$gen_date') AS GEN_DATE,(SELECT STATUS FROM employee_payslip WHERE EMP_PROFILE_ID=employee_profile.PROFILE_ID AND GENERATION_DATE='$gen_date') AS STATUS,(SELECT NETPAY FROM employee_payslip WHERE EMP_PROFILE_ID=employee_profile.PROFILE_ID AND GENERATION_DATE='$gen_date') AS NETPAY FROM employee_profile WHERE employee_profile.ID='$empid' AND employee_profile.PAY_STRUCTURE_ID='$stucId'";
			$result = $this->db->query($sql, $return_object = TRUE)->result_array();
			foreach ($result as $key => $value) {
	  	...
	  	// 	$sql="select ID,PROFILE_ID,DEPT_ID,PAY_STRUCTURE_ID,
			...
			foreach ($result as $key => $value) {
				...
				$sql1="SELECT ID,GENERATION_DATE,EMP_PROFILE_ID,PAYSTRUCTURE_ID,STATUS,NETPAY,
						...
						(SELECT CONCAT(FIRSTNAME,' ',LASTNAME) FROM PROFILE WHERE ID='$empID') AS EMPLOYEE_NAME,
						(SELECT ADMISSION_NO FROM PROFILE WHERE ID='$empID') AS EMPLOYEE_NO
						from employee_payslip where EMP_PROFILE_ID='$empID'";
  				$PayslipList[$key] = $this->db->query($sql1, $return_object = TRUE)->result_array();
	  	...
	  	function fetchPaylipDetailbasedonDept($deptid,$fromdate,$uptodate){
	  		if($deptid!=null && $fromdate==null && $uptodate==null){
	  			...
	  			foreach ($result as  $value) {
	  				$emp_id=$value['PROFILE_ID'];
	  				$sql1="SELECT ID,GENERATION_DATE,EMP_PROFILE_ID,PAYSTRUCTURE_ID,STATUS,NETPAY,
							(SELECT NAME FROM paystructure WHERE ID=PAYSTRUCTURE_ID) AS STRUCURE_NAME,
							(SELECT FREQUENCY FROM paystructure WHERE ID=PAYSTRUCTURE_ID) AS FREQUENCY,
							(SELECT CONCAT(FIRSTNAME,' ',LASTNAME) FROM PROFILE WHERE ID='$emp_id') AS EMPLOYEE_NAME,
							(SELECT ADMISSION_NO FROM PROFILE WHERE ID='$emp_id') AS EMPLOYEE_NO
