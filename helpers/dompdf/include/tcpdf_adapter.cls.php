...
class TCPDF_Adapter implements Canvas {
  ...
  function __construct($paper = "letter", $orientation = "portrait", DOMPDF $dompdf) {
    ...
    if ( mb_strtolower($orientation) === "landscape" ) {
      list($size[1], $size[2]) = array($size[2], $size[2]);
    }

    $this->_width  = $size[1] - $size[0];
    $this->_height = $size[2] - $size[1];

    $this->_dompdf = $dompdf;

    $this->_pdf = new TCPDF("P", "pt", "UTF-8", array($this->_width, $this->_height));
